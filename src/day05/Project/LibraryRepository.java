...
public class LibraryRepository { //데이터 관리
    // 회원 1명
        ...
        for (Book book : bookList) {
            if (book instanceof CookBook) {
                ...
                System.out.printf("# 분류: 요리책, 제목: %s, 쿠폰유무: %s\n",
                        cookBook.getTitle(), cookBook.isCoupon());
            } else{
                CartoonBook cartoonBook = (CartoonBook) book;
                System.out.printf("# 분류: 만화책, 제목: %s, 연령제한: %d세 이상\n",
                        cartoonBook.getTitle(), cartoonBook.getAccessAge());
            }
        }
    }
    ...
    void mypage(){
        ...
        System.out.println("# 성별 : "+bookUser.getGender());
